...
class TestRavenDashboard(unittest.TestCase):
    ...
    def test_logs_pane_in_writer(self):
        ...
        # Observability headline $ = Costs headline $ (same spend / spend_label)
        self.assertIn('id="obsSpend">${spend:.4f}</div>', text)
        self.assertIn('id="costSpend">${spend:.4f}</div>', text)
        self.assertNotIn("Est spend (traces)", text)
        self.assertNotIn('om["est"]', text)
        self.assertIn("same calculator as Costs", text)
        self.assertIn("per-row", text)
        self.assertIn("est_cost_usd", text)
